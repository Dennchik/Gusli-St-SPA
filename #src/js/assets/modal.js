export default function modal() {
	...
	mainSlides.forEach(mainSlide => {
		...
		img.onclick = function (e) {
			let target = e.target;
			let sourse = target.closest('.content-mainslide__image').querySelector(
				'source').srcset;
			let ahref = target.closest('.content-mainslide').querySelector(
				'.content-mainslide__link').href;
			if (!target.closest('.mainslide__slide').classList.contains(
				'swiper-slide-active')) {
				e.preventDefault();
			} else {
